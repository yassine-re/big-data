...
CREATE TABLE IF NOT EXISTS silver.fact_stay_diagnoses (
    ...
    age_at_diagnosis_approx Nullable (UInt8),
...
ORDER BY (
        run_id, stay_sk, code_cim10, diagnosis_type
    );
...
ORDER BY (
        run_id, source_table, record_key, rule_code
    );

-- Compatibilité avec un volume ClickHouse créé avant l'ajout des enrichissements.
ALTER TABLE silver.fact_stays
DROP COLUMN IF EXISTS age_at_admission_approx;

ALTER TABLE silver.fact_stay_diagnoses
ADD COLUMN IF NOT EXISTS patient_sk FixedString (64)
AFTER stay_sk;

ALTER TABLE silver.fact_stay_diagnoses
ADD COLUMN IF NOT EXISTS age_at_diagnosis_approx Nullable (UInt8)
AFTER diagnosis_type;

ALTER TABLE silver.fact_stays
ADD COLUMN IF NOT EXISTS stay_count UInt8 DEFAULT 1
AFTER discharge_mode;

ALTER TABLE silver.fact_stays
ADD COLUMN IF NOT EXISTS is_readmission_30d UInt8 DEFAULT 0
AFTER length_of_stay_minutes;

ALTER TABLE silver.fact_stay_diagnoses
ADD COLUMN IF NOT EXISTS diagnosis_count UInt8 DEFAULT 1
AFTER diagnosis_type;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS reading_count UInt8 DEFAULT 1
AFTER temp_c;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS heart_rate_alert UInt8 DEFAULT 0
AFTER reading_count;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS spo2_alert UInt8 DEFAULT 0
AFTER heart_rate_alert;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS temp_alert UInt8 DEFAULT 0
AFTER spo2_alert;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS is_alert UInt8 DEFAULT 0
AFTER temp_alert;

ALTER TABLE silver.fact_monitoring
ADD COLUMN IF NOT EXISTS alert_rule_version LowCardinality (String) DEFAULT 'monitoring-alert-v1'
AFTER is_alert;
